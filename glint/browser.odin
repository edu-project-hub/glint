...
prepare :: proc(self: ^Glint_Browser) {
	...
	for i in 0 ..< 3 {
		...
		self.texts[i].velocity = {rand.float32_range(-3, 3), rand.float32_range(-3, 3)}
...
update_text_positions :: proc(self: ^Glint_Browser) {
	...
	for i in 0 ..< 3 {
		...
		if self.texts[i].position.x <= 0 ||
		   self.texts[i].position.x + f32(text_width) >= f32(self.window_dims.x) {
		...
		if self.texts[i].position.y <= 0 ||
		   self.texts[i].position.y + f32(text_height) >= f32(self.window_dims.y) {
...
render :: proc(self: ^Glint_Browser, evl: ^app.Event_Loop(Glint_Browser)) -> app.Glint_Loop_Err {
	...
	for i in 0 ..< 3 {
		...
		model = linalg.mul(
			model,
			linalg.matrix4_translate_f32({self.texts[i].position.x, self.texts[i].position.y, 0}),
		)
